        ...
        <form method="post" action="{{ route('stores.update', $store->f_id) }}">
            ...
            <div class="mb-3">
                ...
                <select name="f_accountid" id="f_accountid">
                    @foreach($accounts as $account)
                        <option value="{{ $account->f_id }}" {{ $store->f_accountid === $account->f_id ? 'selected' : '' }}>{{ $account->f_name }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div  class="mb-3">
                ...
                <select name="f_r1id" id="f_r1id">
                    @foreach($r1s as $r1)
                        <option value="{{ $r1->f_id }}" {{ $store->f_r1id === $r1->f_id ? 'selected' : '' }}>{{ $r1->f_name }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div  class="mb-3">
                ...
                <select name="f_r2id" id="f_r2id">
                    @foreach($r2s as $r2)
                        <option value="{{ $r2->f_id }}" {{ $store->f_r2id === $r2->f_id ? 'selected' : '' }}>{{ $r2->f_name }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div  class="mb-3">
                ...
                <select name="f_r3id" id="f_r3id">
                    @foreach($r3s as $r3)
                        <option value="{{ $r3->f_id }}" {{ $store->f_r3id === $r3->f_id ? 'selected' : '' }}>{{ $r3->f_name }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div  class="mb-3">
                ...
                <select name="f_r4id" id="f_r4id">
                    @foreach($r4s as $r4)
                        <option value="{{ $r4->f_id }}" {{ $store->f_r4id === $r4->f_id ? 'selected' : '' }}>{{ $r4->f_name }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div  class="mb-3">
                ...
                <select name="f_r5id" id="f_r5id">
                    @foreach($r5s as $r5)
                        <option value="{{ $r5->f_id }}" {{ $store->f_r5id === $r5->f_id ? 'selected' : '' }}>{{ $r5->f_name }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="mb-3">
                ...
                <select name="f_personid" id="f_personid">
                    @foreach($persons as $person)
                        <option value="{{ $person->f_id }}" {{ $store->f_personid === $person->f_id ? 'selected' : '' }}>{{ $person->f_name }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="mb-3">
                ...
                <select name="f_projectid" id="f_projectid">
                    @foreach($projects as $project)
                        <option value="{{ $project->f_id }}" {{ $store->f_projectid === $project->f_id ? 'selected' : '' }}>{{ $project->f_name }}</option>
                    @endforeach
                </select>
            </div>
